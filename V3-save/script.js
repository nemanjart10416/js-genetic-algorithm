...
var counter = 0;
var lifetime = 1000;
var number_of_chromosomes = 300;

var mutation_chance = 0.020;

var best = 0;

var percent_to_save = 10;
var to_save = (number_of_chromosomes*percent_to_save) / 100;

var ground_height = 1000;
var ground_width = 1000;
var chromosome_dimensions = 5;

var positions = ["down","left","right","up"];
var position_increment = 2.5;
...
var chromosome = function (){
    ...
    this.move = () => {
        ...
        for(var i=0;i<obstacles.length;i++){
            if(
                this.x>obstacles[i].x && (this.x + chromosome_dimensions) < (obstacles[i].x + obstacles[i].width) &&
                this.y>obstacles[i].y && (this.y+chromosome_dimensions) < (obstacles[i].y+obstacles[i].height)
            ){
                this.alive = false;
            }
        }
...
var make_obstacle = () => {
    var arr = [
        new obstacle(400,170,200,20),
        new obstacle(100,170,200,20),
        new obstacle(700,170,200,20),
        new obstacle(150,320,270,20),
        new obstacle(600,320,300,20),
    ];
...
var interval = setInterval(()=>{
    if(counter>=lifetime){
        ...
        if(population[0].y>best){
            best = population[0].y;
        }

        var saved = [];
        for(var i=0;i<to_save;i++){
            population[i].reset_position();
            saved.push(population[i]);
        }
        ...
        for(var i=0;i<saved.length;i++){
            new_population.push(saved[i]);
        }

        console.log("population number "+new_population.length+" saved "+saved.length);
